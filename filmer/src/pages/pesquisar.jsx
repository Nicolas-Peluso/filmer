                        ...
                        <li key={item.id}>
                            ...
                            <p className="pR">{item.overview !== undefined ? item.overview :"conecido por  " + item.known_for_department}</p> <br />
                                <Link to={(item.known_for !== undefined) ? "/pessoa" : "/detail"}>
                                    <button onClick={() => {
                                        item.known_for !== undefined ? props.pessoa(item.id) : props.movie(item.id)}}>ver mais</button>
                                </Link>
...
return props.data !== undefined ? seacrh() : <h1 style={{color: "red", textAlign: "center"}}>voce deve inserir um termo de pesquisa valido</h1>
}
