...
export default function PersonPage(props) {
   ...
   const perosnDetailRight = () => {return (
        <div className="containerPessoa" style={{backgroundColor: "white"}}>
            <ul style={{backgroundColor: "white", display: "inline-block", position: "absolute"}}>
            ...
            <p style={{padding: "10px", fontSize:"1.2em"}}>{result.biography}</p>
           
            <div className="info"> <br />
            <p style={{fontSize:"1.2em", textAlign: "center"}}>nascido em {result.birthday}</p> 
            <p style={{fontSize:"1.2em"}}>{result.deathday !== null ? result.deathday : null}</p>
            <p style={{fontSize:"1.2em"}}>local de nascimento {result.place_of_birth}</p>
            </div></div>
